...
import com.aripd.ecommerce.entity.WishitemEntity_;
...
@Stateless
public class WishitemServiceBean extends CrudServiceBean<WishitemEntity, Long> implements WishitemService {
    ...
    public WishitemServiceBean() {
        super(WishitemEntity.class);
    }

    @Override
    public List<WishitemEntity> findAllByUser(UserEntity user) {
        CriteriaBuilder cb = getEntityManager().getCriteriaBuilder();
        CriteriaQuery<WishitemEntity> cq = cb.createQuery(WishitemEntity.class);
        Root<WishitemEntity> root = cq.from(WishitemEntity.class);

        Predicate predicate = cb.equal(root.get(WishitemEntity_.createdBy), user);
        cq.where(predicate);
    ...
    public WishitemEntity findOneByUserAndProduct(UserEntity user, ProductEntity product) {
        try {
            ...
            Predicate predicate1 = cb.equal(root.get(WishitemEntity_.createdBy), user);
            Predicate predicate2 = cb.equal(root.get(WishitemEntity_.product), product);
